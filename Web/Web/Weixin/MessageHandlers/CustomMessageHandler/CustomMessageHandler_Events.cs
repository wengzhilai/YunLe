...
using Web.Weixin.MessageHandlers.Download;
using Web.Weixin.MessageHandlers.Utilities;
using System.Web.Configuration;

namespace Web.Weixin.MessageHandlers.CustomMessageHandler
{
    /// <summary>
    /// 自定义MessageHandler
    /// </summary>
...
© 2016 Senparc", codeRecord.Version, codeRecord.IsWebVersion ? "网页版" : ".chm文档版");
        ...
        public override IResponseMessageBase OnEvent_ClickRequest(RequestMessageEvent_Click requestMessage)
        {
            ...
            {
                #region 事件
                case "OAuth_help"://免费救援
                    {
                        var strongResponseMessage = CreateResponseMessage<ResponseMessageNews>();
                        strongResponseMessage.Articles.Add(new Article()
                        {
                            Title = "点击【查看全文】进入免费救援",
                            Url = "http://www.cd-cm.com/UserCar/index.html?openid=" + requestMessage.FromUserName + "#/editHelpOrder"
                        });
                        reponseMessage = strongResponseMessage;
                    }
                    break;
                case "Login_UserCar"://点击进入用户端
                    {
                        var strongResponseMessage = CreateResponseMessage<ResponseMessageNews>();
                        strongResponseMessage.Articles.Add(new Article()
                        {
                            Title = "点击【查看全文】进入免费救援",
                            Url = "http://www.cd-cm.com/UserCar/index.html?openid=" + requestMessage.FromUserName
                        });
                        reponseMessage = strongResponseMessage;
                    }
                    break;
                case "OAuth_maintain"://维修保养
                    {
                        var strongResponseMessage = CreateResponseMessage<ResponseMessageNews>();
                        strongResponseMessage.Articles.Add(new Article()
                        {
                            Title = "点击【查看全文】进入维修保养",
                            Url = "http://www.cd-cm.com/UserCar/index.html?openid=" + requestMessage.FromUserName + "#/maintainMap"
                        });
                        reponseMessage = strongResponseMessage;
                    }
                    break;
                case "OAuth_orderList"://保单查询
                    {
                        var strongResponseMessage = CreateResponseMessage<ResponseMessageNews>();
                        strongResponseMessage.Articles.Add(new Article()
                        {
                            Title = "点击【查看全文】进入保单查询",
                            Url = "http://www.cd-cm.com/UserCar/index.html?openid=" + requestMessage.FromUserName + "#/insureList"
                        });
                        reponseMessage = strongResponseMessage;
                    }
                    break;
                case "OAuth_trial"://免费审车
                    {
                        var strongResponseMessage = CreateResponseMessage<ResponseMessageNews>();
                        strongResponseMessage.Articles.Add(new Article()
                        {
                            Title = "点击【查看全文】进入免费审车",
                            Url = "http://www.cd-cm.com/UserCar/index.html?openid=" + requestMessage.FromUserName + "#/trialOrderList"
                        });
                        reponseMessage = strongResponseMessage;
                    }
                    break;
                ...
                case "SubClickRoot_Text":
                    {
                        var strongResponseMessage = CreateResponseMessage<ResponseMessageText>();
                        reponseMessage = strongResponseMessage;
                        strongResponseMessage.Content = "公司致力于以云数据处理为基础，用户体验为原则，物联网技术为支撑，打造汽车商贸、金融、维修、保养、美容、保险、救援等为一体的服务体系，为成都及西南地区车主和汽车服务供应商提供综合性解决方案。";
                    }
                    break;
                ...
                default:
                    {
                        //菜单点击，需要跟创建菜单时的Key匹配
                        var eventKeyArr = requestMessage.EventKey.Split('_');
                        if (eventKeyArr.Length > 1)
                        {
                            switch (eventKeyArr[0])
                            {
                                case "staffCar":
                                    {
                                        var strongResponseMessage = CreateResponseMessage<ResponseMessageNews>();
                                        ...
                                        {
                                            Title = "服务商登录",
                                            Description = "点击【查看全文】进入系统",
                                            Url = string.Format("http://www.cd-cm.com/staffCar/index.html?openid={0}#/{1}", requestMessage.FromUserName, eventKeyArr[1])
                                        });
                                        reponseMessage = strongResponseMessage;
                                    }
                                    break;
                                case "UserCar":
                                    {
                                        ...
                                        strongResponseMessage.Articles.Add(new Article()
                                        {
                                            Title = "用户登录",
                                            Description = "点击【查看全文】进入系统",
                                            Url = string.Format("http://www.cd-cm.com/UserCar/index.html?openid={0}#/{1}", requestMessage.FromUserName, eventKeyArr[1])
                                        });
                                        reponseMessage = strongResponseMessage;
                                    }
                                    break;
        ...
        public override IResponseMessageBase OnEvent_ScanRequest(RequestMessageEvent_Scan requestMessage)
        {
            ...
            if (!string.IsNullOrEmpty(requestMessage.EventKey))
            {
                ...
                #region 下载文档
                if (requestMessage.EventKey.IndexOf("qrscene_") > -1)
                {
        ...
        public override IResponseMessageBase OnEvent_SubscribeRequest(RequestMessageEvent_Subscribe requestMessage)
        {
            ...
            var weixinUser = db.WeixinUser_SingleId(null, ref err, RequestMessage.FromUserName);
            if(weixinUser==null || string.IsNullOrEmpty(weixinUser.OPENID))
            {
                db.WeixinUser_Save(null, ref err, new ProInterface.Models.YL_WEIXIN_USER
                {
                    OPENID = RequestMessage.FromUserName,
                    TAKE_TIME = DateTime.Now,
                    EXIT_TIME = null,
                    SCENE_STR = requestMessage.EventKey,
                    SUBSCRIBE = 1
                }, null);
            }
            else
            {
                db.WeixinUser_Save(null, ref err, new ProInterface.Models.YL_WEIXIN_USER
                {
                    ...
                    SCENE_STR = requestMessage.EventKey,
                    SUBSCRIBE = 1
                }, "TAKE_TIME,EXIT_TIME,SCENE_STR,SUBSCRIBE".Split(','));
            }
        ...
        public override IResponseMessageBase OnEvent_UnsubscribeRequest(RequestMessageEvent_Unsubscribe requestMessage)
        {
            ...
            ProServer.Service db = new ProServer.Service();
            ProInterface.ErrorInfo err = new ProInterface.ErrorInfo();
            db.WeixinUser_Save(null, ref err, new ProInterface.Models.YL_WEIXIN_USER { OPENID = RequestMessage.FromUserName, EXIT_TIME = DateTime.Now, SUBSCRIBE = 0 }, "OPENID,EXIT_TIME,SUBSCRIBE".Split(','));
            #endregion
            return responseMessage;
        }
