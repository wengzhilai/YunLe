...
namespace ProServer
{
    public class ExcelHelper
    {
        ...
        static MemoryStream ExportDT(DataTable dtSource,string sheetName, string strHeaderText="")
        {
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                //如果超过65535条记录，则用多个工作表
                if (rowIndex == 65535 || rowIndex == 0)
                {
                    if (rowIndex != 0)
                    {
                        sheetNum++;
                        sheet = workbook.CreateSheet(sheetName + "_" + sheetNum) as HSSFSheet;
                    }
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    switch (column.DataType.ToString())
                    {
                        case "System.String": //字符串类型
                            ...
                            {
                                if (drValue.Length > 200) drValue = drValue.Substring(0, 200);
        ...
        /// <param name="dtSource">源DataTable</param>
        /// <param name="strHeaderText">表头文本</param>
        public static byte[] ExportDTtoByte(DataTable dtSource, string sheetName=null, string strHeaderText="")
        {
            if (dtSource.Rows.Count > 10000)
            {
                StringBuilder sb = new StringBuilder();
                IList<string> allColumns=new List<string>();
                for(var i=0;i<dtSource.Columns.Count;i++)allColumns.Add(dtSource.Columns[i].Caption);
                sb.AppendLine(string.Join(",", allColumns));
                for (var i = 0; i < dtSource.Rows.Count; i++)
                {
                    sb.AppendLine(string.Join(",", dtSource.Rows[i].ItemArray));
                }
                dtSource.Dispose();
            ...
            {
                using (MemoryStream ms = ExportDT(dtSource, sheetName, strHeaderText))
                {
                    return ms.ToArray();
                }
            }
        ...
        public static DataTable ImportExcel2007toDt(string strFileName)
        {
            ...
            {
                return ImportExcel2003toDt(strFileName);
            }
        }
