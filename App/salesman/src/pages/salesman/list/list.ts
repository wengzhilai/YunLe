...
import {SalesmanInfoPage} from "../info/info";


@Component({
  selector: 'salesman-list',
...
export class SalesmanListPage  {
  ...
  doInfinite(infiniteScroll) : Promise<any> {
    ...
    this.toPostService.ListFun("SalesmanList",this.bean).then((currMsg:any)=>{
      if (currMsg.IsError == true) {
        this.commonService.hint(currMsg.Message)
      } else {
  ...
  GoAdd(){
    this.navCtrl.push(SalesmanEditPage)
  }
  GoInfo(ent:any) {
    // this.navCtrl.push(TabsPage, {ID: ent.ID})
    this.navCtrl.push(SalesmanInfoPage, {id: ent.ID})
  }
  showBigImage(url) {
